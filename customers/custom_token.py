...
class TokenSerializer(TokenObtainPairSerializer):
    def adminPerms(self):
        ...
        perms = Permission.objects.filter(group__id=admin_group_id).values_list(
            "codename", flat=True
        )
    ...
    def userPerms(self, staff_user):
        group_ids = UserAccessGroup.objects.filter(user_id=staff_user.id).values_list(
            "group__id", flat=True
        )
    ...
    def get_token(self, user):
        ...
        try:
            access_groups = UserAccessGroup.objects.filter(user_id=user.id).values_list(
